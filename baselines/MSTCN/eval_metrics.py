...
eval_metrics.py
=====================================================================
Evaluates the trained MSTCN checkpoint (epoch-50, split_1) on the 160
held-out test videos of our_new_dataset, saving everything into THIS
folder only:

    metrics_per_activity.csv   frame Acc / Edit / F1@{10,25,50} per
                                activity (by README.txt id range) +
                                overall (micro over all 160 test videos)
    flops_params.csv           MACs, FLOPs, #parameters (thop, at the
                                mean test-time input length)
    results_table.png / .pdf   one summary table image with metrics +
                                FLOPs + params

Predictions (results/our_dataset/split_1/<id>, no extension) and
ground truth (../../dataset/test/groundtruth/<id>.txt, the shared
repo-root dataset) are already 1:1 frame-aligned -- scored directly,
no up/down-sampling needed.
=====================================================================
"""
import os
import numpy as np
...
# ---------------------------------------------------------------------
num_stages, num_layers, num_f_maps, features_dim = 4, 10, 64, 2048
model = MultiStageModel(num_stages, num_layers, num_f_maps, features_dim, num_classes).to(device)
model.eval()
...
ax.set_title("MS-TCN (official) on our_dataset (split_1, 1920 train / 160 test)",
