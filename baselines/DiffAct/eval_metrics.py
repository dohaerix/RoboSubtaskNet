#!/usr/bin/env python3
"""
eval_metrics.py  (DiffAct)
=====================================================================
Evaluates DiffAct's best checkpoint (epoch-40, highest decoder-agg
test Acc during training) on the 160 held-out test videos of
our_new_dataset. Predictions were regenerated from that checkpoint by
predict_best.py into result/OurNewDataset-Trained-S1/prediction/.

Outputs (saved in THIS folder only):
    metrics_per_activity.csv   frame Acc / Edit / F1@{10,25,50}
    flops_params.csv           params + inference FLOPs.
                               NOTE: DiffAct inference = 1 encoder pass
                               + sampling_timesteps (25) decoder passes,
                               so FLOPs = enc + 25*dec (temporal_aug
                               ensembling ignored; single-pass figure).
...
# dataset/ (see dataset_paths.py) -- this folder keeps no local datasets/ copy.
RESULTS_DIR = os.path.join(HERE, "result", "OurDataset-Trained-S1", "prediction")
...
# ---------------------------------------------------------------------
# metric helpers (identical to the other five models' eval scripts)
# ---------------------------------------------------------------------
def segs(lab):
    if not lab:
        return []
...
for vid_id in test_ids:
    pred_path = os.path.join(RESULTS_DIR, f"{vid_id}.txt")
...
# ---------------------------------------------------------------------
# FLOPs / parameters
# Inference = 1 encoder pass (get_features) + 25 DDIM decoder passes.
# Config: configs/our_dataset.json
# ---------------------------------------------------------------------
with open(os.path.join(HERE, "configs", "our_dataset.json")) as f:
    cfg = json.load(f)
enc_p = cfg["encoder_params"]
dec_p = cfg["decoder_params"]
sampling_timesteps = cfg["diffusion_params"]["sampling_timesteps"]

encoder = EncoderModel(enc_p["num_layers"], enc_p["num_f_maps"], enc_p["input_dim"],
                       num_classes, enc_p["kernel_size"], enc_p["normal_dropout_rate"],
                       enc_p["channel_dropout_rate"], enc_p["temporal_dropout_rate"],
                       feature_layer_indices=enc_p["feature_layer_indices"]).to(device).eval()
backbone_dim = enc_p["num_f_maps"] * len(enc_p["feature_layer_indices"])
decoder = DecoderModel(backbone_dim, num_classes, dec_p["num_layers"], dec_p["num_f_maps"],
                       dec_p["time_emb_dim"], dec_p["kernel_size"], dec_p["dropout_rate"]).to(device).eval()

mean_T = int(round(np.mean(input_lengths))) if input_lengths else 50
x = torch.randn(1, enc_p["input_dim"], mean_T).to(device)
enc_macs, enc_params = profile(encoder, inputs=(x, True), verbose=False)

feats = torch.randn(1, backbone_dim, mean_T).to(device)
t = torch.zeros(1, dtype=torch.long).to(device)
event = torch.rand(1, num_classes, mean_T).to(device)
dec_macs, dec_params = profile(decoder, inputs=(feats, t, event), verbose=False)

params = enc_params + dec_params
macs = enc_macs + sampling_timesteps * dec_macs
flops = 2 * macs
...
ax.set_title("DiffAct (official) on our_dataset (split_1, 1920 train / 160 test, epoch-90)",
