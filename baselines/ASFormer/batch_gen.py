...
class BatchGenerator(object):
    ...
    def read_data(self, vid_list_file):
        ...
        # Store base video ids (not paths) -- features/groundtruth are read
        # from the shared repo-root dataset/ via dataset_paths.py.
        self.gts = [vid.split('.')[0] for vid in self.list_of_examples]
        self.features = [vid.split('.')[0] for vid in self.list_of_examples]
        self.my_shuffle()
    ...
    def next_batch(self, batch_size, if_warp=False): # if_warp=True is a strong data augmentation. See grid_sampler.py for details.
        ...
        for idx, vid in enumerate(batch):
            features = dataset_paths.load_combined_features(batch_features[idx])
            file_ptr = open(dataset_paths.groundtruth_path(batch_gts[idx]), 'r')
