...
def create_dataset(cfg: CfgNode):
    ...
    def load_video(vname):
        ...
        with open(os.path.join(groundTruth_path, vname + '.txt')) as f:
            # some groundTruth files have a spurious extra trailing blank
            # line (double newline); filter blanks instead of relying on
            # a fixed [:-1] slice so those files don't KeyError on ''
