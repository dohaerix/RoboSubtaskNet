#!/usr/bin/env python3
"""
eval_metrics.py  (FACT)
=====================================================================
Evaluates FACT's best checkpoint (iter 38400, chosen by FACT's own
training-time model selection -> best_ckpt.gz) on the 160 held-out
test videos of our_new_dataset.

Predictions were extracted from best_ckpt.gz (each Video object stores
`pred`; expanded to GT length with FACT's own expand_frame_label, the
identical op its compute_metrics uses) into:
    results/our_new_dataset/split_1/<id>.txt

Outputs (saved in the FACT folder only):
    metrics_per_activity.csv, flops_params.csv, results_table.png/.pdf
...
from models.blocks import FACT as FACTModel
...
with open(dataset_paths.split_bundle_path("test")) as f:
    test_ids = [l.strip()[:-4] if l.strip().endswith(".txt") else l.strip() for l in f if l.strip()]
...
for vid_id in test_ids:
    # This checkpoint was trained when the pick_pour task was internally
    # named "pick_pour_new"; its cached predictions (extract_predictions.py
    # output) keep that name, while the shared dataset/ (and this script's
    # own TASKS/GT lookup) use the canonical "pick_pour".
    pred_id = vid_id.replace("pick_pour_", "pick_pour_new_", 1) if vid_id.startswith("pick_pour_") else vid_id
    pred_path = os.path.join(RESULTS_DIR, f"{pred_id}.txt")
...
# ---------------------------------------------------------------------
# FLOPs / parameters (thop) on the FACT model with our trained config
# ---------------------------------------------------------------------
cfg = get_cfg_defaults()
cfg.merge_from_file(os.path.join(HERE, "configs", "our_dataset.yaml"))
in_dim = 2048

model = FACTModel(cfg, in_dim, num_classes).to(device)
model.eval()

mean_T = int(round(np.mean(input_lengths))) if input_lengths else 50
seq = torch.randn(mean_T, in_dim).to(device)
# dummy per-frame class labels (transcript derived internally)
label = torch.zeros(mean_T, dtype=torch.long).to(device)
label[mean_T // 3: 2 * mean_T // 3] = 1
label[2 * mean_T // 3:] = 2

with torch.no_grad():
    macs, params = profile(model, inputs=([seq], [label]), verbose=False)
...
ax.set_title("FACT (official) on our_dataset (split_1, 1920 train / 160 test, iter 96000)",
